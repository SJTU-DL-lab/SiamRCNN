...
class SubDataSet(object):
    ...
    def get_image_anno(self, vid_track, frame, out='keypoints'):
        video, track = vid_track.split('_')
        frame = "{:06d}".format(frame)
        image_path = join(self.root, video, self.path_format.format(frame, track, 'x'))
        image_anno = self.labels[vid_track][frame]
        if out == 'keypoints':
            output = self.labels[vid_track]['kp_'+frame]
        elif out == 'mask':
            output = join(self.root, video, self.mask_format.format(frame, track))

        return image_path, image_anno, output

    def get_val_pair(self, index):
        video_name = self.videos[index]

        return self.get_image_anno(video_name, 0), \
               self.get_image_anno(video_name, 0)
...
class DataSets(Dataset):
    ...
    def __getitem__(self, index, debug=False):
        ...
        # else:
        template, search = dataset.get_val_pair(index)
