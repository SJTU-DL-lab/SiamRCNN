from models.siampose_bibranch import SiamMask
...
class MaskCorr(Mask):
    def __init__(self, oSz=63):
        ...
        self.oSz = oSz
        self.kp_direct_reg = DepthCorr(256, 256, 17*2)
        self.kp_heatmap = DepthCorr(256, 256, oSz**2)

    def forward(self, z, x):
        kp_coord = self.kp_direct_reg(z, x)
        kp_heatmap = self.kp_heatmap(z, x)
...
class Custom(SiamMask):
    ...
    def track_mask(self, search):
        search = self.features(search)
        rpn_pred_cls, rpn_pred_loc = self.rpn(self.zf, search)
        kp_coord, kp_heatmap = self.mask(self.zf, search)
        return rpn_pred_cls, rpn_pred_loc, kp_coord, kp_heatmap
